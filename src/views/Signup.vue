...
<template>
    ...
    <div class="page-body bg-neutral-white border-radius-6">
      <iframe :src="loginlink" @load="load" scrolling="no" height="1000px" width="100%" v-show="showframe"></iframe>
    </div>
...
<script>
  ...
  export default {    
    ...
    data() {
    	return {
          loginlink: "https://competitions.codalab.org/accounts/signup/",
          showframe: true,
          loggedin: false
    	};
    },
    methods: {
       async load(){
          ...
          .then(response => {
            ...
            if (hasStatus){
              // console.log("Yes");
              this.$parent.$parent.$refs.header.loggedin = true
              this.$router.push({ name: 'home' });
              console.log(this.$parent.$parent.$refs.header.loggedin);
              // this.$parent.$refs.header.loggedin;
            }
            else{
